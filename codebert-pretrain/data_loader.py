...
class BertData_adjust(data.Dataset):
    """
    Dataset that has binary samples.
    """
    def __init__(self, file_name):
    ...
    def __getitem__(self, offset):
        ...
        #code = tokenizer.encode(new_ls, return_tensors="pt", padding="max_length",truncation=True,max_length=256)
        # code_attn = code['attention_mask'][0]
        org_code_str = self.data[offset]['code']
        #code_lable = tokenizer(org_code_str, return_tensors="pt", padding="max_length",truncation=True,max_length=256)['input_ids'][0]#org_code_str的tokenizer编码
