...
class RenamesDataset(Dataset):
    def __init__(self, data_file: str, args, training=True) -> None:
        ...
        self.examples_pairs = self.read_examples(args.simi)
    ...
    def read_examples(self, simi = False):
        examples = []
        ...
        with io.open(self.data_file, "r", encoding="utf-8") as f:
            for idx, i in enumerate(f):
                ...
                if self.training:
                    e = (Example(i[0]), Example(i[1]), int(i[2]), int(i[3]))
                elif simi == False:
                    if np.isnan(float(i[2])):
                        continue
                    e = (Example(i[0]), Example(i[1]), float(i[2]))
                elif simi == True:
                    if np.isnan(float(i[3])):
                        continue
                    e = (Example(i[0]), Example(i[1]), float(i[3]))
                examples.append(e)
        return examples
    ...
    def collate_fn_transformers(example_pairs):
        ...
        if len(example_pairs[0]) == 3:
            return ret[0], ret[1], torch.tensor([e[2] for e in example_pairs])
        elif len(example_pairs[0]) == 4:
            return ret[0], ret[1], torch.tensor([e[2] for e in example_pairs]), torch.tensor([e[3] for e in example_pairs])
        else:
            return ret
