...
class BERT(Encoder):
    ...
    def __init__(self, bert_model: str, last_n_layer_output: int = 4, pool_choice : str = 'last_mean'):
        super().__init__()
    ...
    @staticmethod
    def from_args(args):
        return BERT(args.bert_model, args.last_n_layer_output, args.pool_choice)
    ...
    def forward(self, input_ids, attention_mask):
        ...
        if self.pool_choice == 'first_last_mean':
            pooled = ((torch.mean(all_hids[0],dim=1) + torch.mean(all_hids[-1],dim=1)) / 2)
        elif self.pool_choice == 'last_mean':
            pooled = torch.mean(all_hids[-1],dim=1)
        elif self.pool_choice == 'last_head':
            pooled = all_hids[-1][:, 0]
        elif self.pool_choice == 'middle':
            pooled = all_hids[-4][:, 0]
